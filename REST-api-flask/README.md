...
After cloning, create a virtual environment and install the requirements. For Linux and Mac users:
    ...
    ➜ REST-api-flask: virtualenv ootsuk
    ➜ REST-api-flask: source ootsuk/bin/activate
    (ootsuk) ➜ REST-api-flask: pip3 install -r requirements.txt

If you are on Windows, then use the following commands instead:

    ➜ REST-api-flask: virtualenv ootsuk
    ➜ REST-api-flask: ootsuk\Scripts\activate
    (ootsuk) ➜ REST-api-flask: pip3 install -r requirements.txt
...
To run the server use the following command:
    ...
    (ootsuk) ➜ REST-api-flask: python api.py
...
The following `curl` command registers a new user with username `b3ns44d` and password `password`:

    $ curl -i -X POST -H "Content-Type: application/json" -d '{"username":"b3ns44d","password":"password"}' http://127.0.0.1:5000/api/users
...
These credentials can now be used to access protected resources:
    ...
    $ curl -u b3ns44d:password -i -X GET http://127.0.0.1:5000/api/resource
    HTTP/1.0 200 OK
    Content-Type: application/json
    Content-Length: 30
    Server: Werkzeug/0.9.4 Python/2.7.3
...
Using the wrong credentials the request is refused:
    ...
    $ curl -u b3ns44d:password1234 -i -X GET http://127.0.0.1:5000/api/resource
...
Finally, to avoid sending username and password with every request an authentication token can be requested:
    ...
    $ curl -u b3ns44d:password -i -X GET http://127.0.0.1:5000/api/token
    HTTP/1.0 200 OK
    Content-Type: application/json
    Content-Length: 139
    Server: Werkzeug/0.9.4 Python/2.7.3
